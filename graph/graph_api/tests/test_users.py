...
class TestUsersGETFollowings:
    ...
    def test_GET_followings_with_pictures_of_existing_user(self, app: Flask, populate_db: None) -> None:
        ...
        user_being_followed_a = User(email='yes_ucl@kcl.ac.uk', profile_image=str(user_a_image))._asdict()
        ...
        user_being_followed_b = User(email='lello@gmail.com', profile_image=str(user_b_image))._asdict()
        ...
        json = response.get_json()
        user_with_one_following_reduced = {
            'full_name': user_being_followed_a['full_name'], 'email': user_being_followed_a['email'], 'profile_image': literal_eval(user_being_followed_a['profile_image'])}
        user_with_no_followings_reduced = {
            'full_name': user_being_followed_b['full_name'], 'email': user_being_followed_b['email'], 'profile_image': literal_eval(user_being_followed_b['profile_image'])}
        assert len(json) == 2
        #Prepare json response for image assertions
        for person in json:
            person['profile_image'] = literal_eval(person['profile_image'])
            if person['email'] == 'yes_ucl@kcl.ac.uk':
                assert person['profile_image'] == user_a_image
            if person['email'] == 'lello@gmail.com':
                assert person['profile_image'] == user_b_image
