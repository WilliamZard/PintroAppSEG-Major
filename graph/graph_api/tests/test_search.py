...
from .generate_test_data import User, Space, Business, Tag, basic_space_node, basic_business_node, basic_user_node, basic_tag_node
...
class TestPOST:
    ...
    def test_query_that_partially_matches_full_names_or_emails_should_return_correct_nodes(self,app, populate_db):
        ...
        nodes = []
        space_data =[]
        matching_space = Space(email='matching_space@test.com', full_name='Lello Pasqualino')._asdict()
        ...
        business_data=[]
        matching_business = Business(email='matching_business@test.com', full_name='Lello Di Caprio')._asdict()
    ...
    def test_query_that_partially_matches_stories_should_return_correct_nodes(self,app, populate_db):
        ...
        business_data=[]
        matching_business = Business(email='matching_business@test.com', story='It is a business sponsored by UCL.')._asdict()
        business_data.append(matching_business)
        business_data.append(Business(email='business_b@test.com')._asdict())
        business_data.append(Business(email='business_c@test.com')._asdict())
        for business in business_data:
    ...
    def test_query_that_partially_matches_short_bios_should_return_correct_nodes(self,app, populate_db):
        ...
        space_data =[]
        matching_space = Space(email='matching_space@test.com', short_bio='This team makes researches on COVID-19')._asdict()
        space_data.append(matching_space)
        space_data.append(Space(email='space_b@test.com')._asdict())
        space_data.append(Space(email='space_c@test.com')._asdict())
        for space in space_data:    
        ...
        business_data=[]
        matching_business = Business(email='matching_business@test.com', short_bio='We are a company fighting against COVID-19')._asdict()
        business_data.append(matching_business)
        business_data.append(Business(email='business_b@test.com')._asdict())
        business_data.append(Business(email='business_c@test.com')._asdict())
        for business in business_data:
        ...
        user_data=[]
        matching_user = User(email='Lello@test.com', short_bio='I got COVID-19 once.')._asdict()
        user_data.append(matching_user)
        user_data.append(User(email='user_b@test.com')._asdict())
        user_data.append(User(email='user_c@test.com')._asdict())
        for user in user_data:
